...
from .module.metrics import get_sparse_cross_entropy_loss, get_accuracy
...
class CopyNet:
    def __init__(self, config, word_embedding_matrix):
        ...
        y_pred, self.predicted_ids, self.alignment_history = self.forward()
        self.loss = get_sparse_cross_entropy_loss(self.tgt_out, y_pred, mask_sequence_length=self.tgt_len)
        self.accu = get_accuracy(self.tgt_out, y_pred, mask_sequence_length=self.tgt_len)
    ...
    def forward(self):
        ...
        y_pred = self.training_decoding_layer(enc_output, enc_state, self.src_inp, self.src_len, tgt_em)

        # decoding in testing
        if not self.beam_search:
            predicted_ids, alignment_history = self.inference_decoding_layer(
                enc_output, enc_state, self.src_inp, self.src_len, beam_search=self.beam_search
            )
        else:
            # tiled to beam size
            ...
            predicted_ids, alignment_history = self.inference_decoding_layer(
                tiled_enc_output, tiled_enc_state, tiled_src_inp, tiled_src_len, beam_search=self.beam_search
            )

        return y_pred, predicted_ids, alignment_history
    ...
    def training_decoding_layer(self, enc_output, enc_state, src_inp, src_len, tgt_em):
        with tf.variable_scope('decoder', reuse=False):
            ...
            y_pred = final_outputs.rnn_output

        return y_pred

    def inference_decoding_layer(self, enc_output, enc_state, src_inp, src_len, beam_search):
